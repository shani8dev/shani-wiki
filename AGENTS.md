...
- **Superseded by `shani-docs` — notice added to `index.html` (2026-09-26).** `wiki.shani.dev` and `docs.shani.dev` were both live with **zero cross-links in either direction**, so a reader landing here had no way to learn that `shani-docs` is the maintained set. Added a `.warning-box` (`role="alert"`, `fa-exclamation-triangle`) as the first element inside `<main>`, before the Overview section, linking to `https://docs.shani.dev` and stating the content is unmaintained since April 2026. It deliberately reuses the two `.warning-box` blocks already in this file (lines ~1578, ~2784) rather than inventing new styling, so it needs no CSS. Verified: `html5lib` strict parse 0 errors on `index.html` + `404.html`, 96 anchor links with 0 broken, `sitemap.xml` valid, `node --check assets/js/script.js` OK, script-tag count still 1 (so the strict `script-src 'self'` CSP is unaffected), and a real browser load confirming it renders above the fold (1104×140, orange warning styling, link visible). **Inert `frame-ancestors` CSP directive — removed 2026-09-26.** A real browser load surfaced one console error: `The Content Security Policy directive 'frame-ancestors' is ignored when delivered via a <meta> element`. `frame-ancestors` is only honoured as an HTTP response header, so `frame-ancestors 'none'` in this file's CSP meta was never enforced and the intended clickjacking protection was not actually applied — while implying it was. Verified across the ecosystem first: no sibling repo (`shani-docs`, `shani-blog`, `shani-website`) has the directive in a meta, and **none of the four live sites sends a CSP response header at all**, so this file was the only place it appeared. Removed from the meta, which is behaviour-preserving (it was already inert) and clears the console error. **The underlying gap is still open:** this site has no real clickjacking protection, because GitHub Pages cannot send `X-Frame-Options` or a CSP `frame-ancestors` header. Fixing that needs a CDN/proxy in front of Pages or dropping Pages — an infrastructure decision, not a file edit. Do not re-add the directive to the meta; it does nothing and will fail the zero-console-error check.
